...
describe('Notification Middleware', () => {
  ...
  //     response: {
  //       data: {
  //         title: 'Đặt lệnh ban đầu đã tồn tại',
  //         status: 400,
  ...
  // it('should trigger an error toast message and return error for 400 response code', () => {
  //   expect(store.dispatch(TITLE_EXIST).error.response.status).toEqual(400);
  //   const toastMsg = (toastify.toast as any).error.getCall(0).args[0];
  //   expect(toastMsg).toContain('Đặt lệnh ban đầu đã tồn tại');
  // });
