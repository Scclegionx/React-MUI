...
export const getEntities = createAsyncThunk('orders/fetch_entity_list', async ({ page, size, sort }: IQueryParams) => {
  const requestUrl = `${apiUrl +'/find'}${sort ? `?page=${page}&size=${size}&sort=${sort}&` : '?'}cacheBuster=${new Date().getTime()}`;
...
export const updateVwapPrice = createAsyncThunk(
  'orders/updateVWAP',
  async (timeVWAP: string, thunkAPI) => {
    const result = await axios.post<BigInteger>(apiUrl, timeVWAP);
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
...
export const OrdersSlice = createEntitySlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updateVwapPrice.fulfilled, state => {
        state.updating = false;
        state.updateSuccess = true;
        state.entity = {};
      })
      ...
      .addMatcher(isPending(createEntity, updateEntity, partialUpdateEntity, deleteEntity,updateVwapPrice), state => {
        state.errorMessage = null;
        state.updateSuccess = false;
        state.updating = true;
      });
